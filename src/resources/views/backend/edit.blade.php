...
<form class="pure-form pure-form-stacked" action="/backend/{{ $type }}s/{{ $book->id }}/" method="POST">
{{ method_field('PUT') }}
{{ csrf_field() }}
	...
	<div class="pure-u-1-1 pure-u-md-5-8">
		@if ( !isset($thumbnail))
			<fieldset class="">
				...
				<input placeholder="location of file" class="pure-input-1-2" type="text" name="thumbnail[path]" value="{{ $book->thumbnail->path or '' }}">
			    <input placeholder="File name" class="pure-input-1-2" type="text" name="thumbnail[name]" value="{{ $book->thumbnail->name or '' }}">
			    <input placeholder="Photographer" class="pure-input-1-2" type="text" name="thumbnail[photographer]" value="{{ $book->thumbnail->photographer or '' }}">
			    <input placeholder="Link to photographer's profile" class="pure-input-1-2" type="text" name="thumbnail[link]" value="{{ $book->thumbnail->link or '' }}">
			</fieldset>
		@endif
	    ...
	    <fieldset class="pure-group">
	        ...
	        <input name="subtitle" class="pure-input-3-4" type="text" placeholder="Subtitle" value="{{ $book->subtitle->value or '' }}">
		    <input name="description" class="pure-input-1" type="text" placeholder="Description" value="{{ $book->description->value or '' }}" {{ $description or '' }}>
			...
			@foreach ( $roles as $role )
			    ...
			    <script>
				    $(function() {
					    ...
					    var selectedOptions = <?php echo $book->peoples($role)->get()->pluck('id')->toJson(); ?>;
					    $('#{{ $role }}').selectize({
						    persist: false,
						    createOnBlur: true,
						    create: true,
		...
		<fieldset class="">
	        ...
	        <script>
				$(function() {
					...
					var selectedOptions = <?php echo $book->keywords()->pluck('keywords.id')->toJson(); ?>;
				    $('#keywords').selectize({
					    persist: false,
						createOnBlur: true,
						create: true,
	        ...
	        <select name="bundles[]">
		        @foreach ( $categories as $category )
		            ...
		            @if ( $book->bundleSingle('category')->title->value == $category->title->value )
		            	selected
		            @endif
	        ...
	        <input name="timestamp[draft]" type="date" 
	        	@if ( isset($book->timestamp->draft) )
	        		value="{{ $book->timestamp->draft->toDateString() }}"
	        	@endif
	        >
	        <label>Date published</label>
	        <input name="timestamp[publish]" type="date" @if ( isset($book->timestamp->publish) )
	        		value="{{ $book->timestamp->publish->toDateString() }}"
	        	@endif
	        >
	        <label>Date Amend</label>
	        <input name="timestamp[amend]" type="date" @if ( isset($book->timestamp->amend) )
